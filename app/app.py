...
app.layout = dbc.Container([
    ...
    dbc.Button("Reset game", id="reset-game", color="secondary", className="me-1"),
    dbc.Row([
        dbc.Col([
    ...
    dbc.Alert("Agent made action None", id='result-agent', color="success", className="me-1"),
])
...
def make_human_action(n_clicks, human_act, n_clicks_reset):
    ...
    if 'reset-game.n_clicks' in changed_id or n_clicks == 0:
        shutil.copyfile(os.path.join(ENV_DIRECTORY, 'env.pkl'),
                        os.path.join(ENV_DIRECTORY, f'env_{getpass.getuser()}.pkl'))
